...
//            return new Pair<>(BigInteger.ZERO, 0.0);
    ...
    //计算state的期望；
    public static Double computeMeanFitDFS(BigInteger stateID/*,ArrayList<PVAR_INST_DEF> action*/, int searchDeep, long timeLimit) {
        if (shouwMsg)
            System.out.println("getBestAction");
...
//        否则，消耗完时间，同时终止剩下的嵌套；
    ...
    public Double getStateActionFitness(State s, ArrayList<PVAR_INST_DEF> action, Long timeLimit, int actionNum, int searchdeep) {
        ...
        for (BigInteger nextState : nexthStateList) {
            Double stateFit = computeMeanFitDFS(nextState, searchdeep, 10);
            meanFitness += stateFit;
        }
    ...
    public Double getStateFitness(State s, Long time/* ms */, int searchdeep) {
        ...
        //compute next state fitness
        Double stateFit = computeMeanFitDFS(stateID, searchdeep, 10);
