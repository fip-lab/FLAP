...
public abstract class Policy {
	...
	//by yi
	static public void resetPolicy(){


		//conformant related
		expectMaxVarDepth = 10;
		maxVarDepth = 10;
		theRatio = -1;
		ifConformant = true;
		...
		timeHis = new LinkedList<>();
		nodesupdateHis = new LinkedList<>();
		gradientCost = 0;
		fndalhpaCost = 0;
		//constraints for projection
		// built in buildF, used in projection
		...
		ifInSumConstraints = null;
		ifEqual = new ArrayList<>();
		ifConstructConstraints = true;
		// const for random policy
		randomAction = new ArrayList<>();
		countActBits = 0;

		ifInGraph = null;
		ifForcednotChoose = null;
		//minimal probability of action vars
		//caused by XXXX=>move() preconditions
		//used in projection
		...
		//for constraints
		//used for adding additional effects to
		_extraEffects = new HashMap<>();
		_extraEffectsLVARS = new HashMap<>();
		//static public HashMap<PVAR_NAME, HashMap<ArrayList<TYPE_NAME>, ArrayList<BOOL_EXPR>>> _forcingConditions = new HashMap<>();

		_forceActionCondForall = new HashMap<>();
		_forcedCondForallLVARS = new HashMap<>();

		_forceActionCondExist = new HashMap<>();
		_forcedCondExistLVARS = new HashMap<>();

		LVARRecord = new HashMap<>();
		TYPERecord = new HashMap<>();
		superClass = new HashMap<>();
		childClass = new HashMap<>();
		...
		ifPrintSizePredict = false;

		HashMap<PVAR_NAME, HashMap<ArrayList<LCONST>, Integer>> act2Int = new HashMap<>();

		//record each step state in the graph
		//used for retreiving satate variable value
		//when doing projection for XXXXX=>move()
		ArrayList<TEState> stateHistory = new ArrayList<>();
	}
	//add by yi
	public int _horizon = 0;
	public int _totalHorizon = 0;
	public int _actionWideLimit = 0;
	public int _stateWideLimit = 0;
	...
	public void setSearchPara(int actionWide, int stateWide) {
		...
		_stateWideLimit = stateWide;
		_actionWideLimit = actionWide;
	}
	public Double getStateFitness(State s, Long timeLimit, int searchdeep) {
		System.out.println("return null ");
		return 0.0;
	}
	public Double getStateActionFitness(State s, ArrayList<PVAR_INST_DEF> action, Long timeLimit, int actionNum, int searchdeep) {
		System.out.println("return null ");
		return 0.0;
	}
