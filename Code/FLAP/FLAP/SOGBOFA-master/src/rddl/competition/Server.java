...
public class Server implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (args.length > 5) {
                STEP_TIME_ALLOWED = Integer.valueOf(args[5]);
            }
            if (args.length > 6) {
    ...
    public void run() {
        ...
        try {
            ...
            for (; r < numRounds && !OUT_OF_TIME; r++) {
                SOGBOFA.writeFile(requestedInstance, "result/plan.csv");
                if (!executePolicy) {
                    r--;
                }
                ...
                while (true) {
                    ...
                    if (SHOW_ACTIONS && executePolicy) {
                        ...
                        System.out.println("** Actions received: " + ds + ";;action size: " + ds.size());
                    ...
                    SOGBOFA.writeFile(immediate_reward + "," + ds.size() + "," + String.valueOf(ds), "result/plan.csv");
            ...
            for (r = 0; r < numRounds; r++) {
                sumOfErr += Math.pow(rewardArray[r] - aveReward, 2);
            }
            double sd = Math.sqrt(sumOfErr / numRounds);
            if (record != null) {
            ...
            String result = requestedInstance+ "," +clientName + "," + String.valueOf(System.currentTimeMillis() - start_time)
                     + "," + String.valueOf(numRounds)
                    + "," + String.valueOf(averageReward) + "," + maxReward+ "," + standardDeviation + "," + deviation  + "," + roundReward;
            //add to end of the file
            SOGBOFA.writeFile(result, "result-single-sogbofa.csv");
...
//			System.out.println(msg.substring(39));
    ...
    static String createXMLSessionInit(int numRounds, double timeAllowed, double stepTime, Server server) {
        DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
        try {
            DocumentBuilder db = dbf.newDocumentBuilder();
            Document dom = db.newDocument();
