...
public class State implements java.io.Serializable {
    ...
    public void init(HashMap<TYPE_NAME, OBJECTS_DEF> domain_objects,
                     ...
                     ArrayList<PVAR_INST_DEF> nf_nonfluents,//NonFluents
                     ArrayList<PVAR_INST_DEF> i_nonfluents,//Instance
...
//            System.out.println("**************************interm+1********************************");
    ...
    //用于清除计算留下的中间变量；
    public void cleanAdvanceState(boolean clear_observations) throws EvalException {
        _state = _nextState;
        _nextState = new HashMap<PVAR_NAME, HashMap<ArrayList<LCONST>, Object>>();//不能设置成新的map，要进行初始化操作；
        ...
        for (PVAR_NAME p : _interm.keySet())
//            if (_interm.get(p) != null)
            _interm.get(p).clear();
        if (clear_observations)
            for (PVAR_NAME p : _observ.keySet())
//                if (_observ.get(p) != null)
                _observ.get(p).clear();
        computeDerivedFluents();
