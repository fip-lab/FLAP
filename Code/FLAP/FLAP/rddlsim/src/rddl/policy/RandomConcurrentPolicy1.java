...
public class RandomConcurrentPolicy1 extends Policy {
    ...
    public RandomConcurrentPolicy1() {
        super();
    }

    public RandomConcurrentPolicy1(String instance_name) {
        super(instance_name);
    }
...
//		_random.reSeed(System.currentTimeMillis());
        ...
        int[] in_permutation = Permutation.permute(s._alActionNames.size(), _random);
        for (int j = 0; j < in_permutation.length; j++) {

            // Get a random action
            PVAR_NAME p = s._alActionNames.get(in_permutation[j]);
...
//                    System.out.println("step time cost"+(System.currentTimeMillis() - t0));
                    break;
                }
            }
            if (actions.size() == num_concurrent_actions)
                break;
            if ((System.currentTimeMillis() - t0) > _timeAllowed) {
//                System.out.println(_timeAllowed);
//                System.out.println("step time cost"+(System.currentTimeMillis() - t0));
                break;
            }
        }
