...
public class EnhanceRandomPolicy extends Policy {
    ...
    public EnhanceRandomPolicy() {
        super();
    }

    public EnhanceRandomPolicy(String instance_name) {
        super(instance_name);
    }
    ...
    public ArrayList<PVAR_INST_DEF> getActions(State s) throws EvalException {
        ...
        while (true) {
            int temp = (int) (Math.random() * (s._alActionNames.size() - 1));
            if (!randomList.contains(temp)) {
                randomList.add(temp);

                // Get a random action
                PVAR_NAME p = s._alActionNames.get(temp);
                ...
                for (int i = 0; i < index_permutation.length; i++) {
                    ArrayList<RDDL.LCONST> terms = inst.get(index_permutation[i]);
            ...
            if (actions.size() == num_concurrent_actions)
                break;
            if (randomList.size()==(s._alActionNames.size()-1))
                break;
        }
